...
- (void)run:(CADisplayLink *)sender {
    ...
    if (requestedRestoreScreenShot) {
        requestedRestoreScreenShot = NO;
        EJCanvasContext2D *ctx = (EJCanvasContext2D *)self.screenRenderingContext;
        UIImage *restoreImage = [self.undoScreenShotDelegate screenshotToRestore];
        if (restoreImage != nil) {
            [ctx drawImageIntoGL:restoreImage];
        }
    }
